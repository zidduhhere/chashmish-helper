...
export class GoogleDriveService {
   ...
   */
  private async scanPublicFolder(folderId: string): Promise<DriveApiResponse> {
    try {
      ...
      console.warn(
        "Falling back to public folder access - limited functionality"
      );
      ...
      throw new Error(
        "Could not access folder. Please ensure it is publicly accessible."
      );
    }
  }
   ...
   */
  filterImageFiles(files: DriveFile[], allowedTypes: string[]): DriveFile[] {
   ...
   */
  async downloadFile(file: DriveFile): Promise<ArrayBuffer> {
    if (!file.downloadUrl) {
